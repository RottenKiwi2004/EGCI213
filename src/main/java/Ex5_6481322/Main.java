...
class Seafood implements Comparable<Seafood> {
    ...
    public Seafood(String name, String type, int omega3, int cholesterol, double mercury) throws InvalidInputException {
        switch(type.charAt(0)) {
            case 'm':case 'c':case 'f': break;
            default: throw new InvalidInputException("Invalid " + type);
        }
        if (omega3 < 0) throw new InvalidInputException("Invalid " + omega3);
        if (cholesterol < 0) throw new InvalidInputException("Invalid " + cholesterol);
        if (mercury < 0) throw new InvalidInputException("Invalid " + mercury);
        this.name = name;
        this.type = type;
        this.omega3 = omega3;
...
public class Main {
    ...
    public static void main(String[] args) {
        final String path = "src/main/java/Ex5_6481322/";
        String filename = path + "seafoods.txt";
            ...
            try {
                ...
                while (fileScanner.hasNextLine()) {
                    ...
                    try {
                        ...
                        Seafood newSeafood = new Seafood(name, type, omega3, cholesterol, mercury);
